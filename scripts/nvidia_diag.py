...
def main() -> int:
    ...
    print("")
    print("Читать так: 529 «Overloaded» — занят инференс NVIDIA, ключ и "
          "права тут ни при чём. Сбоит только шаг 2 — мешает "
          "response_format. Сбоят 2 и 3, а 4 проходит — не работает "
          "поток. Свободна хоть одна модель из пробы — перегружена не "
          "вся NVIDIA, а конкретная модель, и её можно сменить.")
    return 0

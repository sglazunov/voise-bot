...
# Боевой формат расшифровки (formats.to_txt): метка «[чч:мм:сс]» стоит в шапке
# блока говорящего, сами реплики идут голыми строками. Фикстуры раньше были в
# форме «[мм:сс] Имя: реплика», которой на бою не бывает, — и защита от
# ложного времени, написанная под неё, на настоящих встречах не срабатывала.
TRANSCRIPT = ("[00:01:20] Сергей:\n"
              ...
              "\n"
              "[00:03:00] Мария:\n"
              "Решили — оставляем базовые фильтры.")
...
class TestVerifyProtocol:
    def test_verbatim_quote_confirms_and_paraphrase_rejected(self, monkeypatch):
        # Point order = tasks (1, 2) then decisions (3). Время модель называет
        # заведомо неверное («96:34» на встрече в три минуты) — в протокол
        # обязано попасть время метки той реплики, где нашлась цитата.
        backend = FakeBackend([_ev([
            {"i": 1, "quote": "Кирилл, сделай ревизию фильтров до пятницы", "t": "96:34", "owner_ok": True},
            ...
            {"i": 3, "quote": "оставляем базовые фильтры", "t": "01:07:37", "owner_ok": False},
        ])])
        monkeypatch.setattr(analyze.llm, "get_provider_chain", lambda *a, **k: backend)
        res = verify_protocol(_result(), TRANSCRIPT)
        v = res["verification"]
        assert v["decisions"][0]["ok"] and v["decisions"][0]["t"] == "03:00"
        assert v["tasks"][0]["t"] == "01:20"            # а не «96:34» от модели

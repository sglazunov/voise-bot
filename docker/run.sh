...
# ИНВАРИАНТ: ровно ОДИН процесс uvicorn, без --workers. Очередь задач,
# состояние встреч планировщика, слоты записи (Xvfb + PulseAudio) и кэши
# ключей живут в памяти процесса. Со вторым воркером появятся два
# планировщика — два бота на одну встречу — и слоты будут раздаваться
# дважды. Масштабировать можно только вертикально.

...
_DEFAULTS: dict[str, Any] = {
    ...
    # ссылкой на Телемост: затри её, и на следующую встречу не попадёт ни бот,
    # ни человек. Страховка нужна потому, что поле ищется по ЧАСТИЧНОМУ
    # вхождению имени и промахнуться легко (переименовали поле в воркспейсе —
    # и «Видео встречи» находит не то). Список, а не строка: обычный список
    # в настройках заменяется целиком (как `rec_days`), слияние ему не нужно.

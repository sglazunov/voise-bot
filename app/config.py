...
#
# По умолчанию стоит БЕСПЛАТНЫЙ тариф Groq: gpt-oss-120b даёт 8 000 токенов в
# минуту (и 200 000 в сутки). Это меньше одного куска карты, поэтому на
# бесплатном тарифе Groq для протоколов непригоден — цепочка сама отложит его
# в конец как «тесный» (см. _FallbackChain), но лучше просто выключить его
# через VTX_PROVIDER_DISABLED.
#
# На тарифе разработчика у gpt-oss 250 000 токенов в минуту при окне 131 072 —
# тогда осмысленное значение 120 000 (окно с запасом), и Groq становится самым
# дешёвым движком проекта:
#   VTX_GROQ_TPM=120000
PROVIDER_TPM = {"groq": int(os.getenv("VTX_GROQ_TPM", "8000"))}

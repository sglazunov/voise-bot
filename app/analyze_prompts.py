...
from .llm_base import CACHE_MARK
...
_SCHEMA = (
    ...
    '  "caveats": [{{"item": "что именно в системе сейчас временно / отключено / скрыто / '
    'заглушка / прототипное решение (сказано по ходу демо или рассказа)", "kind": "одно из: '
    'отключено | временно | скрыто | заглушка | прототип | риск", "note": "почему так и что '
    'из этого следует: что сломается, что надо не забыть, где это лежит — 1-2 предложения"}}],\n'
...
_RULES = (
    ...
    "- ПРОШЕДШЕЕ ВРЕМЯ — НЕ ЗАДАЧА. «Я добавил ссылку», «мы уже поставили» — это "
    "done_tasks, а не tasks. Если за предложением следует отказ («Нет, не надо», "
    "«не нужно») — пункт ОТМЕНЁН: не задача и не решение, разве что в conclusions как "
    "«Открыто:». Реплики про самого бота-протоколиста (стоп-слово, «добавь в контекст "
    "бота»), просьбы включить микрофон, оргфраза «напишите руководителю за доступом» — "
    "не задачи встречи, не выписывай их.\n"
    "- detailed ТОЛЬКО ИЗ РЕЧИ. Блоки контекста (серия, постоянный контекст, прошлая "
    "встреча) — справочник: цифры, состав команды, инструменты и планы ИЗ НИХ в "
    "разбор не переносятся, если этого не произнесли на ЭТОЙ встрече. Раздел, "
    "собранный по контексту, — выдумка, и он опаснее пропуска.\n"
    ...
    "только пункты, о СОСТОЯНИИ которых шла речь. Если что-то не сделано ПО РЕШЕНИЮ "
    "(«решили не делать», «в прототипе не нужно») — это status «снято», а не «не сделано»/"
    "«в работе», и причина — в note: осознанный отказ и недоделка для читателя — разные вещи.\n"
    "- caveats: ОГОВОРКИ О СОСТОЯНИИ СИСТЕМЫ, сказанные по ходу демо или рассказа: «это я "
    "пока временно отключил», «сохраняю в JSON, БД не трогал», «тут есть функционал, скрытый "
    "по умолчанию», «третья вкладка — заглушка», «в прототипе так, на проде будет иначе». "
    "Это не решения и не задачи, поэтому раньше они не попадали никуда, а именно их ищет "
    "тот, кто тестирует или подхватывает код. На КАЖДУЮ такую оговорку — item (что именно), "
    "kind из списка и note: почему так и что из этого следует (что сломается при передеплое, "
    "чего не видно в интерфейсе, где это лежит). Не выдумывай следствий, которых не "
    "прозвучало, — но и не сворачивай пять оговорок в одну фразу «базовая функциональность "
    "готова».\n"
    "- decisions: если причина решения прозвучала — формулируй решение ВМЕСТЕ с ней "
    "(«отказаться от логирования в прототипе — он на одного пользователя»): решение без "
    "причины читается как каприз, а с причиной — как принятый риск.\n"
...
_MAP_SCHEMA = (
    ...
    '  "caveats": [{{"t": "мм:сс", "item": "что в системе временно / отключено / скрыто / заглушка / '
    'прототипное решение — сказано по ходу демо", "kind": "отключено | временно | скрыто | заглушка | '
    'прототип | риск", "note": "почему и что из этого следует, 1-2 предложения", '
    '"quote": "дословная фраза, иначе null"}}],\n'
...
# ⚠️ Порядок частей здесь подчинён КЭШУ промпта, и его нельзя мешать.
#
# Кэш префиксный: считается начало запроса. Поэтому шаблон идёт от самого
# устойчивого к самому переменному — правила и схема (одинаковы у всех встреч),
# затем контекст команды (одинаков у всех кусков одной встречи), затем сам
# кусок. Границы помечены `CACHE_MARK`; движок, который кэш не умеет, метку не
# увидит — её вырезает цепочка провайдеров.
#
# Номер куска («часть {i} из {n}») переехал ВНИЗ, к самому фрагменту: в шапке он
# менялся от куска к куску и обесценивал кэш целиком — ради двух цифр
# пересчитывались все 2 900 символов правил на каждом куске.
_MAP_TEMPLATE = (
    "Перед тобой часть расшифровки рабочей встречи (автоматическая, возможны "
    ...
    "Не сделано ПО РЕШЕНИЮ («решили не делать») — это «снято», причина в note.\n"
    "- caveats: всё, что сказано ПО ХОДУ демо о состоянии системы — «временно отключил», "
    "«пока в JSON, БД не трогал», «скрыто по умолчанию», «заглушка», «в прототипе так» — "
    "по одному пункту на каждую оговорку, с дословной фразой в quote. Это ищут тестировщики.\n"
...
_MERGE_TEMPLATE = (
    ...
    "- decisions, tasks, statuses и caveats — ВСЕ пункты обоих отрезков (кроме точных дублей), с их t, "
...
_REDUCE_TEMPLATE = (
    ...
    "- caveats из заметок перенеси в caveats протокола ВСЕ (одна оговорка — один пункт; "
    "склеивать разные оговорки в одну фразу нельзя): это единственное место, где "
    "читатель узнает, что отключено, скрыто или временно;\n"

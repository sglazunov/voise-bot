...
| GET | `/api/providers/nvidia/verify` | Ход перебора — для опроса из интерфейса, пока идёт проверка. |
| POST | `/api/providers/nvidia/verify` | Запустить перебор моделей в фоне и сразу вернуть состояние. |
...
| POST | `/api/automation/recorder/login/action` | Клик, ввод текста, клавиша или переход — внутрь браузера входа. |
| POST | `/api/automation/recorder/login/close` | Закрыть окно входа в Яндекс на сервере. |
| POST | `/api/automation/recorder/login/open` | Открыть сессию входа в Яндекс: браузер на СЕРВЕРЕ, экран — в интерфейсе. |
| GET | `/api/automation/recorder/login/screen` | Текущий экран браузера входа (PNG). |
